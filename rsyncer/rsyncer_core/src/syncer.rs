...
use crate::{clients, storage::LastFMStorage};
...
impl Syncer {
    pub fn new(
        ...
        Syncer { spotify, lastfm, storage, concurrency }
    }
    ...
    pub async fn sync(&self) -> Result<usize> {
        ...
        let tracks = *self.spotify.get_liked_tracks().await?;
        ...
        let processed_track_ids: Vec<_> = *self.storage.get_synced_tracks().await?;
        ...
        let sync_results = iter(unprocessed_tracks)
            .map(|t| async move {
                if !self.lastfm.track_exists(&t).await? {
                    return Err(Error::UnknownTrack(t.id));
                }
                self.lastfm.love_track(&t).await?;
                Ok(t.id)
            })
        ...
        self.storage.mark_tracks_as_synced(unprocessed_track_ids).await?;
        info!("Sync process completed successfully. Synced tracks: {unprocessed_track_ids_len:?}",);
        Ok(unprocessed_track_ids_len)
    }
}
