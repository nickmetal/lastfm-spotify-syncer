    ...
    def get_liked_tracks(self) -> Generator[Dict, None, None]:
        ...
        try:
            while not all_fetched:
                time.sleep(0.2)
                response = self.spotify.current_user_saved_tracks(
                ...
                self.logger.info(
                    f'fetched {fetched_count}/{response["total"]} from Spotify API'
                )
            ...
            raise Exception(
                f"{e}. Try to remove auth cache. Run rm .cache in current dir"
            )

    def get_custom_liked_playlist(
        self, playlist_name: str = "lastfm_liked"
    ) -> Optional[Dict]:
